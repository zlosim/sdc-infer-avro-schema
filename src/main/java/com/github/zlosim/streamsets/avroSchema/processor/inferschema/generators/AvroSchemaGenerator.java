...
import com.github.zlosim.streamsets.avroSchema.processor.inferschema.ErrorsCustomInfer;
...
public class AvroSchemaGenerator extends SchemaGenerator {
  ...
  public List<Stage.ConfigIssue> init(SchemaGeneratorConfig config, Stage.Context context) {
    ...
    for(AvroDefaultConfig defaultConfig : config.avroDefaultTypes) {
      ...
      if(defaultValuesForTypes.containsKey(defaultConfig.avroType.getType())) {
        issues.add(context.createConfigIssue("AVRO", "config.avroDefaultTypes", ErrorsCustomInfer.SCHEMA_GEN_0009, defaultConfig.avroType.getLabel()));
        break;
      }
  ...
  public String generateSchema(Record record) throws OnRecordErrorException {
    ...
    if(!rootField.getType().isOneOf(Field.Type.MAP, Field.Type.LIST_MAP)) {
      throw new OnRecordErrorException(record, ErrorsCustomInfer.SCHEMA_GEN_0003, rootField.getType().name());
    }
  ...
  private Schema.Field schemaFieldForType(
    ...
    Schema simpleSchema = simpleSchemaForType(fieldPath, record, field, fieldName, currentPath, rootRecordName);
  ...
  private Schema complexSchemaForType(
    ...
    Schema simpleSchema = simpleSchemaForType(fieldPath, record, field, fieldName, currentPath, null);
    Schema finalSchema = simpleSchema;
   ...
   */
  private Schema simpleSchemaForType(String fieldPath, Record record, Field field, String fieldName, String currentPath, String rootRecordName) throws OnRecordErrorException {
    switch (field.getType()) {
      ...
      case DATETIME:
        ...
        dateTimeSchema.addProp("logicalType", "timestamp-millis");
      ...
      case LIST:
        ...
        if(field.getValueAsList().isEmpty()) {
          throw new OnRecordErrorException(record, ErrorsCustomInfer.SCHEMA_GEN_0006, fieldPath);
        }
        ...
        for(Field listItem : field.getValueAsList()) {
          Schema currentListItemSchema = complexSchemaForType(fieldPath + "[" + index + "]", record, listItem, fieldName, currentPath, rootRecordName + "-beloo");
          if(itemSchema == null) {
            itemSchema = currentListItemSchema;
          } else if (!itemSchema.equals(currentListItemSchema)) {
            throw new OnRecordErrorException(record, ErrorsCustomInfer.SCHEMA_GEN_0005, fieldPath, itemSchema, currentListItemSchema);
          }
          index++;
        }
      ...
      case LIST_MAP:
        ...
        if(field.getValueAsMap() == null || field.getValueAsMap().isEmpty()) {
          throw new OnRecordErrorException(record, ErrorsCustomInfer.SCHEMA_GEN_0008, fieldPath);
        }
...
//            throw new OnRecordErrorException(record, ErrorsCustomInfer.SCHEMA_GEN_0007, fieldPath, mapSchema, currentListItemSchema);
//          }
//        }
        /**
         *
        ...
        for(Map.Entry<String, Field> item : field.getValueAsMap().entrySet()) {
          recordFields.add(schemaFieldForType(
              ...
              item.getKey(),
              item.getValue(),
              item.getKey() + (currentPath.isEmpty() ? "" : '.' + currentPath),
              rootRecordName.isEmpty()? fieldName : rootRecordName

          ));

        }

        Schema recordSchema = Schema.createRecord(rootRecordName.isEmpty()? fieldName : rootRecordName, null, currentPath, false);
        recordSchema.setFields(recordFields);
...
//        return Schema.createMap(mapSchema);
      ...
      case FILE_REF:
      default:
        throw new OnRecordErrorException(record, ErrorsCustomInfer.SCHEMA_GEN_0002, field.getType());
    }
  }
  ...
  private int getDecimalScaleOrPrecision(
    ...
    if(finalValue < minAllowed) {
      throw new OnRecordErrorException(record, ErrorsCustomInfer.SCHEMA_GEN_0004, finalValue, field);
    }
